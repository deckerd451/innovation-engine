...
-- this object already exists and is in active use. Originally merged
-- via PR #209 (2026-05-04), with fixes from PR #215 ("Nearify Event"
-- default label instead of "Unknown Event") and PR #216 (dedupe one
-- row per event/peer pair via best_edges, and use community's actual
-- image_url/bio columns instead of the earlier avatar_url/headline
-- names) already folded in below — this is the final, as-deployed
-- version, not the original PR #209 diff.
--
-- Returns the current user's Nearify interaction peers, grouped by
-- event. Used by the "People you met" profile panel
-- (assets/js/nearify-events-panel.js).
--
