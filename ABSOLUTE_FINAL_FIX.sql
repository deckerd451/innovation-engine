-- ================================================================
-- ABSOLUTE FINAL FIX - NO SKILL MATCHING
-- ================================================================
-- This version removes skill matching entirely to avoid array issues
-- Copy and run in: https://supabase.com/dashboard/project/hvmotpzhliufzomewzfl/sql/new
-- ================================================================
  ...
  SELECT json_build_object(
    ...
    'immediate_actions', json_build_object(
      ...
      'unread_messages', json_build_object(
        'count', (
          SELECT COALESCE(COUNT(DISTINCT conversation_id), 0)::int
          FROM messages m
    ...
    'momentum', json_build_object(
      ...
      'streak', json_build_object(
        ...
        'last_login', c.last_login,
        'is_at_risk', COALESCE(c.last_login < NOW() - INTERVAL '20 hours', true)
      ),
    ...
    'network_insights', json_build_object(
      ...
      'growth', json_build_object(
        'new_connections', (
          SELECT COALESCE(COUNT(*), 0)::int FROM connections
          WHERE (from_user_id = user_community_id OR to_user_id = user_community_id)
          AND status = 'accepted'
          AND created_at > NOW() - INTERVAL '30 days'
        ),
        'new_projects', (
          SELECT COALESCE(COUNT(*), 0)::int FROM project_members pm
          JOIN projects p ON p.id = pm.project_id
          WHERE pm.user_id = auth_user_id
          AND pm.joined_at > NOW() - INTERVAL '30 days'
        ),
        'new_themes', (
          SELECT COALESCE(COUNT(*), 0)::int FROM theme_participants
