...
final class BeaconRegistryService {
    ...
    func refreshBeacons() async throws {
        ...
        cache.beacons = Dictionary(uniqueKeysWithValues: beacons.map { ($0.beaconKey, $0) })
    ...
    private func loadCacheFromDisk() {
        guard let data = UserDefaults.standard.data(forKey: cacheKey),
              let decoded = try? JSONDecoder().decode(BeaconCache.self, from: data) else {
            return
        }
        cache = decoded
