...
function setupEventBridges() {
  ...
  unifiedNetworkApi.on('focus-cleared', ({ previousNodeId, source } = {}) => {
    window.ExplorerCoordinator?.syncFocusedPerson?.(null);
    if (source === 'background' && previousNodeId) {
      const panelEntity = window.getOpenNodePanelEntity?.();
      if (panelEntity?.type === 'person' && String(panelEntity.id) === String(previousNodeId)) {
        window.closeNodePanel?.();
      }
    }
  });
  ...
  unifiedNetworkApi.on('reset-to-my-network', () => {
    window.GraphController?.clearHighlight?.();
    window.SynapseFilter?.set?.('all');
    window.ExplorerCoordinator?.setGraphFilter?.('all');
