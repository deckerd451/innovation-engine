...
export class CoordinationDetectorV2 {
  ...
  async detectMomentumShifts(profile, context, signals) {
    ...
    for (const projectId of context.projectIds || []) {
      ...
      // Note: Uses generated column project_id (from details->>'project_id')
      ...
      if (recentError) {
        console.warn(`⚠️ Momentum detection query failed for project ${projectId}:`, recentError.message);
        continue;
      }
      ...
      if (olderError) {
        console.warn(`⚠️ Momentum detection query failed for project ${projectId}:`, olderError.message);
        continue;
      }
      ...
      // Log query result (one-time per project)
      if (recentCount > 0 || olderCount > 0) {
        console.log(`📊 activity_log query ok for project ${projectId.slice(0, 8)}..., recent=${recentCount}, older=${olderCount}`);
      }
