...
const COLORS = {
  ...
  edgeAccepted: '#00ff88',
  edgePending: '#ffaa00',
  edgeSuggested: 'rgba(0, 224, 255, 0.25)',  // Increased from 0.15 to 0.25 for 10% total (0.25 * 0.4 = 0.1)
...
function startSimulation() {
  ...
  linkElements = container.append('g')
    ...
    .attr('opacity', d => d.status === 'suggested' ? 0.4 : 0.8);  // 0.25 * 0.4 = 0.1 (10% visible)
...
async function addSkillEndorsement(userId, skill) {
  ...
  try {
    ...
    const { data: endorsedUser, error: endorsedUserError } = await supabase
      .from('community')
      .select('user_id')
      .eq('id', userId)
      .single();
    ...
    if (endorsedUserError || !endorsedUser) {
      throw new Error('Could not find user to endorse');
    }
    
    const { data: endorserUser, error: endorserUserError } = await supabase
      .from('community')
      .select('user_id')
