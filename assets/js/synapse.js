...
function startSimulation() {
  ...
  nodeElements.append('circle')
    .attr('r', d => {
      ...
      // Check if this node has a suggested connection to current user
      const isSuggested = links.some(link => 
        link.status === 'suggested' && 
        (link.source.id === d.id || link.target.id === d.id)
      );
      
      // Check if connected (has shouldShowImage flag)
      const isConnected = d.shouldShowImage && !d.isCurrentUser;
      
      if (isConnected) {
        return 28; // Connected users - full size
      } else if (isSuggested) {
        return 21; // Suggested - 25% smaller (28 * 0.75 = 21)
      } else {
        return 14; // Non-suggested - 50% smaller (28 * 0.5 = 14)
      }
    })
