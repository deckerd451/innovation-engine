...
// -----------------------------
// Stats emitter (for dashboard bar)
// -----------------------------
function computeSynapseStats() {
  const peopleCount = nodes.filter(n => !n.type || n.type === 'person').length;
  const projectCount = nodes.filter(n => n.type === 'project').length;
  const suggestedLinksCount = links.filter(l => (l.type === 'auto' || l.status === 'suggested')).length;

  // Connections involving current user (best-effort; falls back to all)
  const myConnections = (connectionsData || []).filter(c =>
    c.from_user_id === currentUserCommunityId || c.to_user_id === currentUserCommunityId
  );

  // Prefer "accepted/active" when present; otherwise count all
  const acceptedStatuses = new Set(['accepted', 'active', 'connected', 'approved']);
  const myAccepted = myConnections.filter(c => !c.status || acceptedStatuses.has(String(c.status).toLowerCase()));
  const myConnectionCount = (myAccepted.length > 0 ? myAccepted.length : myConnections.length);

  // Projects in table (if present) - derived from nodes
  return {
...
function emitSynapseStats(reason = 'update') {
  try {
    const stats = computeSynapseStats();
    // Make it easy for other modules to read without imports
    window.__synapseStats = { ...stats, reason, ts: Date.now() };
    window.dispatchEvent(new CustomEvent('synapse:stats', { detail: window.__synapseStats }));
  } catch (e) {
    console.warn('⚠️ emitSynapseStats failed:', e);
  }
}

// Public getter for modules that prefer polling instead of events
export function getSynapseStats() {
  return window.__synapseStats || computeSynapseStats();
}
...
export async function initSynapseView() {
  ...
  // Push counts to dashboard
  emitSynapseStats('data-loaded');
  ...
  // Initial paint stats
  emitSynapseStats('simulation-started');
...
function drawProjectCircles() {
  // Ensure we don't accumulate duplicate circle layers
  try { container.selectAll('g.project-circles').remove(); } catch (_) {}
