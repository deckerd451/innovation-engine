...
async function addSkillEndorsement(userId, skill) {
  ...
  try {
    // Validate that both users exist in community table
    console.log('Validating endorsement:', {
      endorser: currentUserCommunityId,
      endorsed: userId,
      skill: skill
    });
    ...
    console.log('Both users validated, inserting endorsement');
    
    // Insert endorsement using community IDs directly
    const { error } = await supabase
      .from('endorsements')
      .insert({
        endorsed_id: userId,                          // Community ID
        endorsed_community_id: userId,                // Community ID (not "keep for compatibility")
        endorser_id: currentUserCommunityId,          // Community ID
        endorser_community_id: currentUserCommunityId, // Community ID (not "keep for compatibility")
        skill: skill
      });
