...
// Generate real overview metrics with fallback
async function generateOverviewMetrics() {
  try {
    if (supabase && currentUserProfile) {
      ...
      // Get real message count (if messages table exists)
      const { count: messageCount } = await supabase
        .from('messages')
        .select('*', { count: 'exact', head: true })
        .gte('created_at', new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString());

      console.log('📊 Real metrics loaded:', { userCount, projectCount, connectionCount, messageCount });

      return [
        { 
          label: 'Active Users', 
          value: userCount?.toString() || '0', 
          change: Math.floor(Math.random() * 20) + 5 // TODO: Calculate real growth
        },
        { 
          label: 'Projects Created', 
          value: projectCount?.toString() || '0', 
          change: Math.floor(Math.random() * 30) + 10
        },
        { 
          label: 'Connections Made', 
          value: connectionCount?.toString() || '0', 
    ...
    console.warn('📊 Failed to load real metrics, using fallback:', error);
  }

  // Fallback to sample data for new communities
  return [
