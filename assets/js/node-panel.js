...
// ======================================================================
// Global Profile Editor Bridge (Synapse + Node Panel)
// Ensures any "Edit Profile" button calls openProfileEditor successfully.
// ======================================================================
(function () {
  const MODAL_ID = "ie-edit-profile-modal";
  ...
  function ensureModal() {
    ...
    modal.innerHTML = `
      <div style="
        ...
        border-radius: 18px;
        box-shadow: 0 18px 70px rgba(0,224,255,0.18);
        overflow: hidden;
      ">
        <div style="display:flex; align-items:center; justify-content:space-between; padding:18px 18px; border-bottom:1px solid rgba(0,224,255,0.18);">
          <div style="color:#00e0ff; font-weight:900; letter-spacing:.3px;">
            <i class="fas fa-user-edit"></i> Edit Profile
          </div>
          <button id="${MODAL_ID}-close" style="
        ...
        <form id="${MODAL_ID}-form" style="padding:18px; display:grid; gap:14px;">
          <div style="display:grid; gap:8px;">
            <label style="color:#ffd700; font-weight:800;">Name</label>
            <input name="name" type="text" placeholder="Your name" style="padding:12px; border-radius:12px; border:1px solid rgba(0,224,255,0.25); background: rgba(255,255,255,0.06); color:#fff;">
          </div>

          <div style="display:grid; gap:8px;">
            <label style="color:#ffd700; font-weight:800;">Skills (comma-separated)</label>
            <input name="skills" type="text" placeholder="e.g. Product, Design, AI" style="padding:12px; border-radius:12px; border:1px solid rgba(0,224,255,0.25); background: rgba(255,255,255,0.06); color:#fff;">
          </div>

          <div style="display:grid; gap:8px;">
            <label style="color:#ffd700; font-weight:800;">Bio</label>
            <textarea name="bio" rows="4" placeholder="A short bio..." style="padding:12px; border-radius:12px; border:1px solid rgba(0,224,255,0.25); background: rgba(255,255,255,0.06); color:#fff; resize: vertical;"></textarea>
          </div>

          <div style="display:grid; gap:8px;">
            <label style="color:#ffd700; font-weight:800;">Availability</label>
            <input name="availability" type="text" placeholder="e.g. Evenings, Weekends" style="padding:12px; border-radius:12px; border:1px solid rgba(0,224,255,0.25); background: rgba(255,255,255,0.06); color:#fff;">
          </div>

          <div style="display:grid; gap:8px;">
            <label style="color:#ffd700; font-weight:800;">Image URL</label>
            <input name="image_url" type="url" placeholder="https://..." style="padding:12px; border-radius:12px; border:1px solid rgba(0,224,255,0.25); background: rgba(255,255,255,0.06); color:#fff;">
          </div>

          <div style="display:flex; gap:10px; justify-content:flex-end; padding-top:6px;">
            <button type="button" id="${MODAL_ID}-cancel" style="
              background: rgba(255,255,255,0.10);
              border: 1px solid rgba(255,255,255,0.18);
            ...
            <button type="submit" id="${MODAL_ID}-save" style="
              background: linear-gradient(135deg,#00e0ff,#0080ff);
              border: none; color: #001018;
              border-radius: 12px;
              padding: 10px 16px; cursor: pointer; font-weight:900;
              box-shadow: 0 10px 22px rgba(0,224,255,0.22);
            ">Save</button>
          </div>

          <div id="${MODAL_ID}-hint" style="min-height:18px; color:#aaa; font-size:13px;"></div>
        </form>
      </div>
    `;
    ...
    modal.querySelector(`#${MODAL_ID}-cancel`).addEventListener("click", close);

    modal.querySelector(`#${MODAL_ID}-form`).addEventListener("submit", async (e) => {
      e.preventDefault();
      const hint = modal.querySelector(`#${MODAL_ID}-hint`);
      const btn = modal.querySelector(`#${MODAL_ID}-save`);

      const supabase = window.supabase;
      if (!supabase) {
        hint.textContent = "Supabase client not available.";
        return;
      }

      // Prefer the globally shared profile if present
      const profile = window.currentUserProfile || window.__currentUserProfile || window.appState?.communityProfile;
      const profileId = profile?.id;

      if (!profileId) {
        hint.textContent = "Profile not loaded yet.";
        return;
      }
      ...
      try {
        ...
        window.__currentUserProfile = data;
        if (window.appState) window.appState.communityProfile = data;

        hint.style.color = "#00e0ff";
        hint.textContent = "Saved ✓";

        window.dispatchEvent(new CustomEvent("profile-updated", { detail: { profile: data } }));

        setTimeout(() => (modal.style.display = "none"), 300);
      } catch (err) {
        console.error("Edit profile save failed:", err);
        hint.style.color = "#ff6b6b";
        hint.textContent = "Save failed: " + (err?.message || err);
      } finally {
        btn.disabled = false;
        btn.style.opacity = "1";
      }
  ...
  function fillForm(modal) {
    ...
    const p = window.currentUserProfile || window.__currentUserProfile || window.appState?.communityProfile || {};
    form.elements.name.value = p.name || "";
    form.elements.skills.value = p.skills || "";
  ...
  // Compatibility aliases (Synapse may call one of these)
  window.openEditProfile = window.openProfileEditor;
  window.openOnboarding = window.openProfileEditor;
  window.startOnboarding = window.openProfileEditor;
  window.showOnboarding = window.openProfileEditor;

  console.log("✅ Profile editor bridge ready (openProfileEditor/openEditProfile/openOnboarding)");
})();
