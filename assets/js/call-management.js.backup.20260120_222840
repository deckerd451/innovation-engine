...
window.openVideoCallInterface = function() {
  console.log('📹 Opening video call interface - PRODUCTION MODE');
  
  // Check if already open
  if (document.getElementById('video-call-interface')) {
    console.log('Video call interface already open');
    return;
  }
  ...
  if (window.showSynapseNotification) {
    window.showSynapseNotification('Video call interface ready', 'success');
  }
  
  // Track video call start
  if (typeof window.trackEvent === 'function') {
    window.trackEvent('video_call_started', { 
      call_id: window.currentCall.id,
      mode: 'production'
    });
  }
};
