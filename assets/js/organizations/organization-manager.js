 ...
 * Queries the organizations table directly (same pattern as getOpportunities).
...
export async function getOrganizations(filters = {}) {
  try {
    let query = supabase
      .from("organizations")
      .select("id, name, slug, description, logo_url, industry, size, location, follower_count, opportunity_count, verified, status, created_at")
      .eq("status", "active");
