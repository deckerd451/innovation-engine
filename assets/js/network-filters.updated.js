...
// Ensure the UI exists before any open/close/toggle calls (prevents null.style crashes)
function ensurePanel() {
  if (!overlay || !filterPanel) {
    try { createFilterPanel(); } catch (e) {
      console.warn("⚠️ NetworkFilters: unable to create panel:", e);
    }
  }
}
...
// ================================================================
// Compatibility: bind bottom-bar FILTERS button (id="btn-filters")
// This prevents dashboardPane.js legacy toggleFilters() from firing.
// ================================================================
function bindFiltersButton() {
  const btn = document.getElementById("btn-filters");
  if (!btn) return;

  // Remove any existing handler we might have attached (idempotent)
  if (btn.__networkFiltersBound) return;
  btn.__networkFiltersBound = true;

  btn.addEventListener(
    "click",
    (e) => {
      // Stop other click handlers (e.g., dashboardPane toggleFilters) from running.
      e.preventDefault();
      e.stopPropagation();
      if (typeof e.stopImmediatePropagation === "function") e.stopImmediatePropagation();

      // Ensure panel exists then toggle
      try {
        if (!filterPanel) createFilterPanel();
        toggleFilterPanel();
      } catch (err) {
        console.error("❌ Failed to open Network Filters panel:", err);
      }
    },
    true // capture so we win the race
  );
}

// Ensure binding runs even if module loads before DOM is ready
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", bindFiltersButton);
} else {
  bindFiltersButton();
}

// Provide legacy global names some older code may call
...
// --------------------------------------------------------------------
// Stable API surface (so other modules can safely call filters)
// --------------------------------------------------------------------
window.NetworkFilters = window.NetworkFilters || {};
window.NetworkFilters.open = openFilterPanel;
window.NetworkFilters.close = closeFilterPanel;
window.NetworkFilters.toggle = toggleFilterPanel;
window.NetworkFilters.apply = applyFilters;
window.NetworkFilters.clear = clearAllFilters;
window.NetworkFilters.getState = () => JSON.parse(JSON.stringify(activeFilters));

// Named exports (optional; keeps this usable via dynamic import)
export function initNetworkFilters() { init(); }
export function openFilters() { openFilterPanel(); }
export function closeFilters() { closeFilterPanel(); }
export function toggleFilters() { toggleFilterPanel(); }
export function getFiltersState() { return JSON.parse(JSON.stringify(activeFilters)); }
