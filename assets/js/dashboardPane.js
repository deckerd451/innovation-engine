...
(function () {
  ...
  function wireGlobalFunctions() {
    ...
    window.openQuickConnectModal = async () => {
      ...
      await loadSuggestedConnections({ shouldOpenModal: false });
    };
...
async function initSynapseOnce() {
  ...
  async function loadSuggestedConnections(options = {}) {
  const {
  ...
  // Ensure modal exists / open
  if (shouldOpenModal) openModal('quick-connect-modal');

  const listEl = document.getElementById("suggested-connections-list");
  if (!listEl) return;

  // Lightweight loading UI
  listEl.innerHTML = `
    <div class="qc-loading">
      <div class="qc-loading-title">Finding your best next connections…</div>
      <div class="qc-loading-sub">Matching skills, interests, and network proximity</div>
    </div>
  `;

  try {
    const core = await import("./synapse/core.js").then((m) => m).catch(() => null);
    if (!core?.getRecommendations || !core?.getSynapseStats) {
      listEl.innerHTML = `<div class="qc-empty">Synapse recommendations not available yet.</div>`;
      return;
    }

    const stats = core.getSynapseStats();
    const meId = stats?.currentUserCommunityId || null;

    // Fetch recommendations from Synapse (which delegates to pathway-animations)
    const recs = await core.getRecommendations({ limit: 10 }).catch(() => []);
    const recommendations = Array.isArray(recs) ? recs : [];

    // Persist stepper state for next/prev + autoplay
    pathwayStepper.core = core;
    pathwayStepper.meId = meId;
    pathwayStepper.recommendations = recommendations;
    pathwayStepper.index = 0;

    if (!meId) {
      listEl.innerHTML = `<div class="qc-empty">Create your profile before viewing recommendations.</div>`;
      return;
    }

    if (!recommendations.length) {
      listEl.innerHTML = `<div class="qc-empty">No recommendations yet — try adding more skills/interests, or check back after more people join.</div>`;
      // Clear any previous pathways
      try { core.clearConnectPathways?.(); } catch (_) {}
      stopPathwayAutoplay();
      return;
    }

    renderSuggestedConnectionsStepper();

    // Always reveal the first pathway immediately
    revealCurrentPathway({ duration: 2000 });

    // Optional autoplay sequence
    if (autoplay) startPathwayAutoplay({ limit: autoplayLimit, intervalMs: autoplayIntervalMs });
  } catch (e) {
    console.error("Suggested connections failed:", e);
    listEl.innerHTML = `<div class="qc-empty">Could not load recommendations.</div>`;
  }
}

/* ==========================================================================
   Suggested Connections — Sequential Stepper (replaces list UI)
   ========================================================================== */
...
function stopPathwayAutoplay() {
  if (pathwayStepper.autoplayTimer) clearInterval(pathwayStepper.autoplayTimer);
  pathwayStepper.autoplayTimer = null;
  pathwayStepper.autoplayRunning = false;
  const btn = document.getElementById("qc-autoplay-toggle");
  if (btn) btn.textContent = "Auto";
}

function startPathwayAutoplay({ limit = 5, intervalMs = 2600 } = {}) {
  stopPathwayAutoplay();

  const max = Math.min(limit, pathwayStepper.recommendations.length);
  if (max <= 1) return;

  pathwayStepper.autoplayRunning = true;
  const btn = document.getElementById("qc-autoplay-toggle");
  if (btn) btn.textContent = "Stop";

  // Start from current position; advance until limit reached
  let shown = 1;

  pathwayStepper.autoplayTimer = setInterval(() => {
    if (!pathwayStepper.autoplayRunning) return;

    // If we reached the requested number, stop.
    if (shown >= max) {
      stopPathwayAutoplay();
      return;
    }

    pathwayStepper.index = Math.min(pathwayStepper.index + 1, pathwayStepper.recommendations.length - 1);
    renderSuggestedConnectionsStepper();
    revealCurrentPathway({ duration: 2000 });
    shown++;
  }, intervalMs);
...
function renderSuggestedConnectionsStepper() {
  const listEl = document.getElementById("suggested-connections-list");
  if (!listEl) return;

  const recs = pathwayStepper.recommendations || [];
  const i = Math.max(0, Math.min(pathwayStepper.index || 0, recs.length - 1));
  const rec = recs[i];

  // Normalize fields from pathway-animations recommendations
  const name = rec?.name || "Unknown";
  const reason = rec?.reason || rec?.why || "";
  const matchedSkills = Array.isArray(rec?.matchedSkills) ? rec.matchedSkills : [];
  const pathDistance = Number.isFinite(rec?.pathDistance) ? rec.pathDistance : null;
  const score = Number.isFinite(rec?.score) ? Math.round(rec.score) : null;

  const pct = recs.length ? Math.round(((i + 1) / recs.length) * 100) : 0;
  const typeLabel = rec?.type === "project" ? "Project" : "Person";

  const skillChips = matchedSkills.slice(0, 6).map((s) => `<span class="qc-chip">${escapeHtml(s)}</span>`).join("");
  const distanceText =
    pathDistance === 1 ? "Direct" :
    pathDistance === 2 ? "2 hops" :
    pathDistance === 3 ? "3 hops" :
    pathDistance != null && pathDistance < 99 ? `${pathDistance} hops` :
    "New";

  listEl.innerHTML = `
    <div class="qc-stepper">
      <div class="qc-stepper-header">
        <div class="qc-stepper-title">Suggested Connection</div>
        <div class="qc-stepper-meta">${i + 1} / ${recs.length} • ${pct}%</div>
      </div>

      <div class="qc-step-card">
        <div class="qc-step-card-top">
          <div class="qc-step-name">${escapeHtml(name)}</div>
          <div class="qc-step-type">${typeLabel}</div>
        </div>

        <div class="qc-step-stats">
          <span class="qc-stat"><span class="qc-stat-label">Network</span> <span class="qc-stat-val">${distanceText}</span></span>
          ${score != null ? `<span class="qc-stat"><span class="qc-stat-label">Score</span> <span class="qc-stat-val">${score}</span></span>` : ""}
        </div>

        ${reason ? `<div class="qc-step-reason">${escapeHtml(reason)}</div>` : ""}

        ${skillChips ? `<div class="qc-chip-row">${skillChips}</div>` : ""}

        <div class="qc-step-actions">
          <button id="qc-connect-btn" class="qc-btn qc-btn-primary">
            Connect
          </button>
          <button id="qc-open-node-btn" class="qc-btn">
            View
          </button>
        </div>
      </div>

      <div class="qc-stepper-controls">
        <button id="qc-prev-btn" class="qc-btn" ${i === 0 ? "disabled" : ""}>Prev</button>
        <button id="qc-next-btn" class="qc-btn" ${i >= recs.length - 1 ? "disabled" : ""}>Next</button>
        <button id="qc-autoplay-toggle" class="qc-btn qc-btn-ghost">${pathwayStepper.autoplayRunning ? "Stop" : "Auto"}</button>
        <button id="qc-clear-path-btn" class="qc-btn qc-btn-ghost">Clear</button>
      </div>

      <div class="qc-stepper-hint">
        The pathway highlights *why* this person/project is a strong next connection. Use Connect to send a request.
      </div>
    </div>
  `;
  ...
  const autoBtn = document.getElementById("qc-autoplay-toggle");
  const clearBtn = document.getElementById("qc-clear-path-btn");
  const connectBtn = document.getElementById("qc-connect-btn");
  const openBtn = document.getElementById("qc-open-node-btn");

  prevBtn?.addEventListener("click", () => {
    stopPathwayAutoplay();
    pathwayStepper.index = Math.max(0, pathwayStepper.index - 1);
    renderSuggestedConnectionsStepper();
    revealCurrentPathway({ duration: 1800 });
  });

  nextBtn?.addEventListener("click", () => {
    stopPathwayAutoplay();
    pathwayStepper.index = Math.min(recs.length - 1, pathwayStepper.index + 1);
    renderSuggestedConnectionsStepper();
    revealCurrentPathway({ duration: 1800 });
  });

  autoBtn?.addEventListener("click", () => {
    if (pathwayStepper.autoplayRunning) stopPathwayAutoplay();
    else startPathwayAutoplay({ limit: 5, intervalMs: 2600 });
  });

  clearBtn?.addEventListener("click", () => {
    stopPathwayAutoplay();
    try { pathwayStepper.core?.clearConnectPathways?.(); } catch (_) {}
  });

  connectBtn?.addEventListener("click", async () => {
    const r = pathwayStepper.recommendations?.[pathwayStepper.index];
    if (!r?.userId) return;

    // Prefer the existing dashboard global helper
    try {
      if (typeof window.sendConnectionRequest === "function") {
        await window.sendConnectionRequest(r.userId, r.type || "person");
      } else {
        console.warn("sendConnectionRequest not available on window");
      }
      // Keep the pathway visible; optionally advance
  ...
  openBtn?.addEventListener("click", () => {
    const r = pathwayStepper.recommendations?.[pathwayStepper.index];
    if (!r?.userId) return;
    // Reuse the click behavior elsewhere: open node panel if present
    try {
      window.openNodePanel?.({ id: r.userId, type: r.type || "person", name: r.name });
    } catch (_) {}
  });
}

function revealCurrentPathway(opts = {}) {
  const core = pathwayStepper.core;
  const meId = pathwayStepper.meId;
  const rec = pathwayStepper.recommendations?.[pathwayStepper.index];

  if (!core || !meId || !rec?.userId) return;
  ...
  try {
    core.showConnectPathways?.(meId, rec.userId, opts);
  } catch (e) {
    console.warn("showConnectPathways failed:", e);
  }
}
...
function escapeHtml(str) {
  ...
  // This is the UI entry-point for the "Illuminate Pathways" button.
  // Desired behavior:
  // 1) Open the Connect modal
  // 2) Show *one* suggested connection at a time with context + CTA
  // 3) Reveal pathways sequentially (autoplay), but allow manual next/prev
  try {
    openQuickConnectModal();

    // Collapse bottom bar for clean viewing (existing behavior)
    try {
    ...
    console.error("Illuminate Pathways failed:", e);
    showToast?.("Could not illuminate pathways", "error");
  }
}
  ...
  // Legacy entry-point (older code expected a list).
  // We now show a sequential stepper UI.
  pathwayStepper.recommendations = Array.isArray(recommendations) ? recommendations : [];
  pathwayStepper.index = 0;
  renderSuggestedConnectionsStepper();
}
