...
(function () {
  ...
  async function init() {
    ...
    // Synapse init hook (supports multiple Synapse implementations)
// This file won't break if Synapse isn't present.
try {
  if (!state.synapseInitialized) {
    // Common patterns across your repo iterations:
    // 1) window.initSynapseView(profile) or window.initSynapseView()
    // 2) window.synapse.init(profile) or window.synapse.init()
    // 3) window.Synapse?.init(profile)
    // 4) window.initSynapse(profile)
    const maybeProfile = state.communityProfile || null;

    if (typeof window.initSynapseView === "function") {
      window.initSynapseView(maybeProfile);
      state.synapseInitialized = true;
    } else if (window.synapse && typeof window.synapse.init === "function") {
      window.synapse.init(maybeProfile);
      state.synapseInitialized = true;
    } else if (window.Synapse && typeof window.Synapse.init === "function") {
      window.Synapse.init(maybeProfile);
      state.synapseInitialized = true;
    } else if (typeof window.initSynapse === "function") {
      window.initSynapse(maybeProfile);
      state.synapseInitialized = true;
    } else {
      // No-op: Synapse module not present or not yet loaded
    }
  }
} catch (e) {
