...
// -----------------------------
  ...
  // -----------------------------
  // Synapse -> Bottom Stats wiring
  // -----------------------------
  function applySynapseStats(detail) {
    if (!detail) return;
    // Projects: count of project nodes in Synapse
    if (typeof detail.projectCount === "number") setStatValue("projects", detail.projectCount);

    // Network: prefer "myConnectionCount" (connections involving current user),
    // fallback to peopleCount (community size in graph)
    if (typeof detail.myConnectionCount === "number") setStatValue("network", detail.myConnectionCount);
    else if (typeof detail.peopleCount === "number") setStatValue("network", detail.peopleCount);
  }

  // Listen for events emitted by /assets/js/synapse.js
  window.addEventListener("synapse:stats", (e) => applySynapseStats(e.detail));

  // If Synapse already emitted before this module loaded, pick it up:
  if (window.__synapseStats) applySynapseStats(window.__synapseStats);
