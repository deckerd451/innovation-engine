...
async function initSynapseOnce() {
  ...
  // -----------------------------
  async // ================================================================
// ILLUMINATE PATHWAYS (SEQUENTIAL RECOMMENDATIONS)
// ================================================================

let __pathwaySequence = {
  active: false,
  recs: [],
  idx: 0,
  fromId: null,
...
async function getSynapseCoreModule() {
  // Prefer cached global (set by main.js or previous calls)
  if (window.__synapseCore) return window.__synapseCore;

  // Some builds may expose it differently
  if (window.SynapseCore) return window.SynapseCore;
  if (window.synapseCore) return window.synapseCore;

  // Last resort: dynamic import
  const mod = await import("/assets/js/synapse/core.js");
  window.__synapseCore = mod;
  return mod;
}

function ensurePathwaySequencePanel() {
  let panel = document.getElementById("pathway-seq-panel");
  if (panel) return panel;

  panel = document.createElement("div");
  panel.id = "pathway-seq-panel";
  panel.className = "modal hidden"; // stays consistent with existing modal CSS hooks
  panel.style.cssText = `
    position: fixed;
    right: 18px;
    bottom: 90px;
  ...
  panel.innerHTML = `
    <div style="display:flex; align-items:center; justify-content:space-between; gap:10px;">
      <div style="display:flex; flex-direction:column; gap:2px;">
        <div id="pathway-seq-title" style="font-weight:800; letter-spacing:.3px;">Illuminate Pathways</div>
        <div id="pathway-seq-sub" style="font-size:12px; opacity:.8;">Finding the best next connection…</div>
      </div>
      <button id="pathway-seq-close" class="btn btn-ghost" style="padding:6px 10px;">✕</button>
    </div>

    <div id="pathway-seq-card" style="margin-top:12px; padding:12px; border-radius:14px; background: rgba(0,224,255,0.07); border: 1px solid rgba(0,224,255,0.15);">
      <div style="display:flex; align-items:center; gap:10px;">
        <div id="pathway-seq-avatar" style="width:44px; height:44px; border-radius:50%; background: rgba(0,224,255,0.18); display:flex; align-items:center; justify-content:center; font-weight:800;">?</div>
        <div style="min-width:0;">
          <div id="pathway-seq-name" style="font-weight:800; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">—</div>
          <div id="pathway-seq-reason" style="font-size:12px; opacity:.85; margin-top:2px; line-height:1.25;">—</div>
        </div>
      </div>

      <div id="pathway-seq-skills" style="margin-top:10px; display:flex; gap:6px; flex-wrap:wrap;"></div>

      <div style="display:flex; gap:10px; margin-top:12px;">
        <button id="pathway-seq-connect" class="btn btn-primary" style="flex:1;">Connect</button>
        <button id="pathway-seq-next" class="btn btn-secondary" style="flex:1;">Next</button>
      </div>

      <div style="margin-top:10px; font-size:11px; opacity:.75;">
        <span id="pathway-seq-progress">0 / 0</span>
        <span style="opacity:.6;"> • </span>
        <span style="opacity:.6;">Tip: click a node for full profile</span>
      </div>
    </div>
  `;
  ...
  // Wire close
  panel.querySelector("#pathway-seq-close")?.addEventListener("click", () => stopPathwaySequence(true));

  // Wire next
  panel.querySelector("#pathway-seq-next")?.addEventListener("click", () => advancePathwaySequence());

  // Wire connect
  panel.querySelector("#pathway-seq-connect")?.addEventListener("click", async () => {
    const rec = __pathwaySequence.recs?.[__pathwaySequence.idx];
    if (!rec?.userId) return;

    // Prefer the existing request function if present
    if (typeof window.sendConnectionRequest === "function") {
      await window.sendConnectionRequest(rec.userId, rec.name || "Connection", "pathway-sequence");
      showToast?.(`✅ Connection request sent to ${rec.name || "this person"}`);
    } else {
      // Fallback: open node panel if available
      if (typeof window.openNodePanel === "function") window.openNodePanel(rec.userId);
      showToast?.("Open the node panel to connect.");
    }
  });
...
function openPathwaySequencePanel() {
  const panel = ensurePathwaySequencePanel();
  panel.classList.remove("hidden");
  return panel;
}

function closePathwaySequencePanel() {
  const panel = document.getElementById("pathway-seq-panel");
  if (panel) panel.classList.add("hidden");
}

function renderPathwaySequenceStep() {
  const panel = ensurePathwaySequencePanel();
  const rec = __pathwaySequence.recs?.[__pathwaySequence.idx];

  const titleEl = panel.querySelector("#pathway-seq-title");
  const subEl = panel.querySelector("#pathway-seq-sub");
  const nameEl = panel.querySelector("#pathway-seq-name");
  const reasonEl = panel.querySelector("#pathway-seq-reason");
  const avatarEl = panel.querySelector("#pathway-seq-avatar");
  const skillsEl = panel.querySelector("#pathway-seq-skills");
  const progEl = panel.querySelector("#pathway-seq-progress");
  const connectBtn = panel.querySelector("#pathway-seq-connect");
  const nextBtn = panel.querySelector("#pathway-seq-next");

  const total = __pathwaySequence.recs?.length || 0;
  progEl.textContent = `${Math.min(__pathwaySequence.idx + 1, total)} / ${total}`;

  if (!rec) {
    titleEl.textContent = "Illuminate Pathways";
    subEl.textContent = "No suggestions right now — try Search, or click a node.";
    nameEl.textContent = "—";
  ...
  titleEl.textContent = "Illuminate Pathways";
  subEl.textContent = "Here’s a high-signal next connection.";
  nameEl.textContent = rec.name || "Suggested connection";
  reasonEl.textContent = rec.reason || "Recommended based on your profile and the network.";
  avatarEl.textContent = (rec.name || "?").trim().charAt(0).toUpperCase();

  // Skills chips
  const skills = Array.isArray(rec.matchedSkills) ? rec.matchedSkills.slice(0, 5) : [];
  skillsEl.innerHTML = skills
    .map(s => `<span style="font-size:11px; padding:4px 8px; border-radius:999px; background: rgba(0,224,255,0.18); border: 1px solid rgba(0,224,255,0.25);">${escapeHtml(String(s))}</span>`)
    .join("");

  connectBtn.disabled = false;
  nextBtn.disabled = total <= 1;

  // Optional: clicking the card opens node panel if available
  panel.querySelector("#pathway-seq-card")?.addEventListener("dblclick", () => {
    if (typeof window.openNodePanel === "function" && rec.userId) window.openNodePanel(rec.userId);
  }, { once: true });
}

async function drawCurrentPathway() {
  const core = __pathwaySequence.core;
  const rec = __pathwaySequence.recs?.[__pathwaySequence.idx];
  if (!core || !rec?.userId || !__pathwaySequence.fromId) return;

  try {
    core.clearConnectPathways?.();
    core.showConnectPathways?.(__pathwaySequence.fromId, rec.userId, { duration: 2000 });
  } catch (e) {
    console.warn("Pathway draw failed:", e);
  }
}

async function advancePathwaySequence() {
  if (!__pathwaySequence.active) return;

  if (__pathwaySequence.timer) {
    clearTimeout(__pathwaySequence.timer);
    __pathwaySequence.timer = null;
  }

  const total = __pathwaySequence.recs?.length || 0;
  if (!total) return;

  __pathwaySequence.idx = Math.min(__pathwaySequence.idx + 1, total - 1);
  renderPathwaySequenceStep();
  await drawCurrentPathway();

  // Auto-advance (sequential reveal), but let users click "Next" any time.
  if (__pathwaySequence.idx < total - 1) {
    __pathwaySequence.timer = setTimeout(() => advancePathwaySequence(), 2600);
  }
}

function stopPathwaySequence(hardClose = false) {
  __pathwaySequence.active = false;
  if (__pathwaySequence.timer) {
    clearTimeout(__pathwaySequence.timer);
    __pathwaySequence.timer = null;
  }
  try {
    __pathwaySequence.core?.clearConnectPathways?.();
  } catch {}
  if (hardClose) closePathwaySequencePanel();
}

async function startPathwaySequence({ limit = 5 } = {}) {
  stopPathwaySequence(false);

  const core = await getSynapseCoreModule();
  __pathwaySequence.core = core;

  const stats = core.getSynapseStats?.() || {};
  const fromId = stats.currentUserCommunityId;
  if (!fromId) {
    showToast?.("Couldn’t identify your community profile yet. Try again in a second.");
    return;
  }

  const recs = await core.getRecommendations?.({ limit });
  if (!Array.isArray(recs) || recs.length === 0) {
    openPathwaySequencePanel();
    __pathwaySequence.recs = [];
    __pathwaySequence.idx = 0;
    __pathwaySequence.fromId = fromId;
    renderPathwaySequenceStep();
    return;
  }

  __pathwaySequence.active = true;
  __pathwaySequence.recs = recs;
  __pathwaySequence.idx = 0;
  __pathwaySequence.fromId = fromId;
  ...
  // Auto-advance sequentially
  if (recs.length > 1) {
    __pathwaySequence.timer = setTimeout(() => advancePathwaySequence(), 2600);
  }
}

// Handler wired to the "Illuminate Pathways" UI button.
// We keep the sequential reveal, but the UI now explains each connection + provides a Connect CTA.
async function showAnimatedPathways() {
  try {
    await startPathwaySequence({ limit: 5 });
  } catch (e) {
    console.error("Illuminate Pathways failed:", e);
    showToast?.("Illuminate Pathways failed — check console for details.");
  }
}
