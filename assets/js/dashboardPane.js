...
async function initSynapseWhenReady(reason = "unknown") {
  ...
  const tryInit = () => {
    // Return a function to call (truthy) if available
  ...
  try {
    const fn = await waitFor(tryInit, { timeout: 15000, interval: 250 });
    console.log(`🧠 Synapse init (${reason}) →`, fn.name || "anonymous");
    fn();
    state.synapseInitialized = true;
  } catch (e) {
    // Not fatal; dashboard can still work without Synapse
