...
function bindEditProfileDelegation() {
  ...
  try {
    // Prefer the stable global API (network-filters.js sets this)
    if (window.NetworkFilters && typeof window.NetworkFilters.toggle === "function") {
      window.NetworkFilters.toggle();
      return;
    }

    // Fallback: lazy-load the module, then toggle
    ...
    // Last resort: call legacy globals if present
    if (typeof window.toggleFilterPanel === "function") {
      window.toggleFilterPanel();
      return;
    }

    console.warn("⚠️ Filters not available (network-filters.js did not initialize).");
  } catch (e) {
    console.warn("⚠️ Filters toggle failed:", e);
  }
}
