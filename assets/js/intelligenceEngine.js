...
async function _fetchCoreGraph(supabase, { communityId, windowDays, now, debug }) {
  ...
  try {
    ...
    if (!oppErr) {
      ...
      console.debug('[intelligence] opportunities loaded:', opportunities.length, '— source: public.opportunities');
    } else {
      console.warn('[intelligence] opportunities query error:', oppErr.message);
    }
  } catch (err) {
    console.warn('[intelligence] opportunities fetch failed:', err.message);
  }
