  ...
  console.log("✅ Login system initialized (OAuth)");
    ...
    function updateUserUI(profile) {
      ...
      if (profile.image_url) {
        document.getElementById('user-avatar-header').innerHTML = `<img src="${profile.image_url}" alt="${name}">`;
      }
    ...
    async function loadMessaging() {
      ...
      try {
        ...
        const { data: convData, error: convError } = await window.supabase
          .from('conversations')
          .select('*, participant_1:community!conversations_participant_1_id_fkey(id, name, image_url), participant_2:community!conversations_participant_2_id_fkey(id, name, image_url)')
          .or(`participant_1_id.eq.${currentUserProfile.id},participant_2_id.eq.${currentUserProfile.id}`)
        ...
        return `
          <div class="project-card" style="background: rgba(0,224,255,0.05); border: 1px solid rgba(0,224,255,0.2); border-radius: 12px; padding: 1.5rem; transition: all 0.3s ease;">
            <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 1rem;">
              <h3 style="color: #00e0ff; margin: 0; flex: 1;">${project.name}</h3>
            ...
            <div style="display: flex; justify-content: space-between; align-items: center;">
              <span style="color: ${statusColor}; font-size: 0.9rem; font-weight: bold;">
                <i class="fas fa-circle" style="font-size: 0.5rem;"></i> ${project.status.replace('-', ' ').toUpperCase()}
              </span>
    ...
    async function updateProject(projectId) {
      ...
      try {
        ...
        if (index !== -1) {
          projects[index] = { ...projects[index], name, description, status, tags };
        }
